ToDo: separate in different notes
Use vimwiki for raw minutes first

### 14 April 2026

- Progress update:
	- Materials shared by Yann and Clara
		- SPC membership? 
		- Clara's comment about shifting MedAccess approach, partly because of capital constraints it seems => also got feedback from Victoria that they do need to raise capital to be able to do more (including with GARDP it seems) 
		- Where are they seeing AMR fitting into their work? In the evolving global health landscape [[Call with Victoria (MedAccess)]]
	- Picking up inception report (a little bit behind schedule)
	- Check on overall timeline
	- Follow-up other consultations (Carol?); I've also followed up with Carmen and Rahul but so far no response
		- Regina meeting => Jeff told her about me
- Any update re Yann (maybe too soon)
- EIB outreach query => Peter contact and get clearance to reconnect with Gavi contacts
- This week:
	- Continue to go through materials => generate Qs
	- Shape inception report, with some adaptation based on what I'm learning
	- Outreach: speaking to Victoria tomorrow, reaching out to Milken this week
	- FAB?
- Peter: comes up with a thousand ideas => manage him
- Anthony Randazzo:
	- Jeff to send holding email for a few weeks 
Admin:
Escalate travel; can also coordinate with someone else eg, Clara if easier

### 21 April 2026

- Progress update:
	- Meeting with Regina Osih and market-shaping

### 19 May 2026

# 202606191030_Jeff_catchup.md

### Notes

- Updated Inception Report and actual report being prepared in parallel
- Still awaiting Regina's market shaping => will reach out to her
	- I'll get started on categories of market shaping tools but also don't want to duplicate what Regina is doing
	- Any prep for MT retreat?
- Billing update
	- Closing out report date => _to be discussed after retreat_
	- **Will send first invoice soon**
- **Drop email where I can provide additional value** => _to be discussed after retreat_
- AMR conference and Milken
- Circle back with Anthony (DFC) based on preliminary terms of Orchid, caveated as subject to negotiation, etc.
	- I'll run key items by Yann and Jeff
- World Bank call with Naomi Rupasinghe ==> Follow-up on 1 Jul (I'll provide a handover if my project is finished by then and more than happy to make the connection also)
- Follow-ups, in process 
	- Follow-ups with IFC, EIB
	- MedAccess
- Hotel headphones 