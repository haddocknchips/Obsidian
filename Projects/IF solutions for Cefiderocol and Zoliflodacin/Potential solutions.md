### Overall

- Financial toolkit plus capabilities and partners that GARDP can deploy depeding on programme and need
- Will be initially shaped by Cefi and Zoli
- Are volume guarantees/ APAs the best solution or is it more effient to look into a revenue + volume hybrid subscription model instead?
	- How do you determine the optimal procuremnt/ goods-linked revenue model? 	
	- How do you generate revenues from low usage because of better stewardship, better health outcomes, etc?
- Basis for portfolio approach?
- Financial modelling clarity, level of detail and standardisation (Cf. Cefiderocol COGS)

### Cefiderocol

- Yann: GARDP was "open to investing" but tax issue. _What is the issue?_ Is this an area that could be looked at again eg, thorugh an SPV or ring-fenced balance sheet, perhaps managed like a fund? 
- Yann: is an Advance Purchase Commitment or some other pull mechanism viable? 
- PS: What if a better solution is ***certainty of revenue*** instead of a [[Cefi concept note - GARDP, MedAccess, Sida|volume guarantee]]?
- APA as preferred procurement mechanism for commercial batches _to derisk manufacturing of validation batches_ ![[APA for Orchid Manufactured Cefiderocol_GMPR Presentation_Dec 2025_v1 1.pptx]] 
	- Who is responsible for execution?
- COGs transaparency and accuracy: is there a widely-accepted third party that can conduct this? [[rahul notes]]

### Zoliflodacin

- Current financing ideas on the table: ![[2025.09 - Strategy on Zoli (2026_2032).pptx]]
	- 1. Funding mechanism to APSL
	- 2. Funding mechanism to DrL to use DrL as MAH subcontractor
	- 3. Loan to DrL
- COGs reduction from current c.[[Yann intro meeting 2026-04-09#^57b98a|$40]] to target $10-20, and eventually $5-10 in 2030/31 => any value in a frontoading/ IFFIm-like mechanism? Some credit enhancement backed by a sovereign funded by a philanthrophy?  

### Strategy / financial sustainability

- APA SPV, or hybrid fund SPV, potentially alongside AMR action fund
- Shift from all downside to downside + upside => one-way to two-way financial model or risk-return symmetry without profit maximisation
- Any potential links with AMR Action Fund?

### IF products and mechanisms (cross matrix with cefi and zoli)

- Typical pull mechanisms:
	- Market entry rewards (MER)
	- Subscription models => how applicable is this to the two products' cost structures eg, fixed vs variable costs [[rahul notes]]
	- Revenue guarantees
	- Enhanced reimbursement
	- Regulatory activities (eg, GAIN market exclusivity extension -- limited pull)
- ***Need to test!*** decoupling revenue from volumes. This _might_ unlock things
	- Check out the [[PASTEUR Act (US)]] as a template
	- [[UK Antibiotic Subscription Model]]
	- [[Sweden’s Reimbursement - Revenue Guarantee Model]]
	- How much do these incentise innovation vs "keeping the drug in the market"?
	- None of these are LMIC-inclusive?
- Hybrid fund/ SPV
	- What can be built out from [[SPV-fundbased pull hybrid|this]]?
- Per Yann, other ideas discussed without being explored
	-  